...
/// 背景卡片堆叠方向
enum CardlyStackDirection {
    case top     // 背景卡片向上偏移（从顶部露出）
    case bottom  // 背景卡片向下偏移（从底部露出）
}

/// 卡片动画引擎，负责拖拽变换、滑出、回弹、缩放等所有动画效果
final class CardlyAnimator {

    // MARK: - 动画配置参数

    /// 滑动触发阈值（0~1），拖拽超过此比例即判定为有效滑动
    var swipeThreshold: CGFloat = 0.4
    /// 拖拽时最大旋转角度
    var maxRotationAngle: CGFloat = .pi / 8
    /// 滑出动画时长
    var swipeOutDuration: TimeInterval = 0.3
    /// 回弹动画时长
    var snapBackDuration: TimeInterval = 0.25
    /// 下一张卡片放大动画时长
    var nextCardScaleDuration: TimeInterval = 0.2
    /// 背景卡片缩放比例（1.0 表示无缩放，越小背景卡越小）
    var backgroundCardScale: CGFloat = 0.95
    /// 背景卡片垂直偏移量（单位 pt，配合 stackDirection 决定偏移方向）
    var backgroundCardVerticalOffset: CGFloat = 12
    /// 背景卡片堆叠方向（.bottom = 从底部露出，.top = 从顶部露出）
    var stackDirection: CardlyStackDirection = .bottom
    ...
    func applyDragTransform(to card: UIView, translation: CGPoint, containerWidth: CGFloat) {
    ...
    func dragProgress(translation: CGPoint, containerWidth: CGFloat) -> (progress: CGFloat, direction: CardlySwipeDirection) {
    ...
    func applyBackgroundTransform(to card: UIView, progress: CGFloat) {
    ...
    func initialBackgroundTransform() -> CGAffineTransform {
        let directionMultiplier: CGFloat = stackDirection == .top ? -1 : 1
    ...
    func animateSwipeOut(card: UIView, direction: CardlySwipeDirection, containerWidth: CGFloat, completion: @escaping () -> Void) {
        isAnimating = true
    ...
    func animateSnapBack(card: UIView, backgroundCard: UIView?, completion: @escaping () -> Void) {
        isAnimating = true
    ...
    func animateNextCardScaleUp(_ card: UIView, completion: (() -> Void)? = nil) {
